	...
	try {
		...
		if (primaryKey !==  '') {
			...
			model.find(primaryKey).then(function (data) {
				...
				return RSVP.Promise.cast(data.prePostHook()).then(function (data) {
			...
			RSVP.Promise.cast(model.create(request.payload).prePostHook()).then(function (data) {
				return data.save();
			}).then(function (data) {
				...
				if (err.name === 'ValidationException')
					throw Hapi.error.badRequest(err.message)
				throw err;
			});
		}
	}
